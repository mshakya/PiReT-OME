...
#unless (  (-e "$workdir/$sample/mapping_results/forward.sorted.prokaryote_ref.bedgraph") && (-e "$workdir/$sample/mapping_results/backward.sorted.prokaryote_ref.bedgraph") ) {next;}
...
open (IN2,"$forwardpileupfile") or die " $forwardpileupfile !";
while(<IN2>)
{
   chomp;
   @array=split /\s+/,$_;
...
{
my $tmpfile=$_;
 open (my $fh, "$tmpfile") or next  "can not open  $tmpfile $! \n";
        while (<$fh>)
  {
   chomp;
...
$/=">";
my %refseq;
    open (my $fh, $reffile) or die "$! $reffile";
          while (<$fh>)
              {
                      chomp;
                      my ($id, $seq) = split /\n/, $_,2;
...
open (OUTG1,">$workdir/sum_direction_prokaryote_ref.gff") or die " can not open description file $workdir/sum_direction_prokaryote_ref.gff $!";
open (OUTG2,">$workdir/sum_direction_prokaryote_ref.fasta") or die " can not open description file $workdir/sum_direction_prokaryote_ref.fasta $!";
foreach  (sort keys %forwardcoverage)
{
             ...
             if ( $forwardcoverage{$id}{$pos}>$mincov || $backwardcoverage{$id}{$pos}>$mincov )
    {
             if ( $forwardcoverage{$id}{$pos} >= $backwardcoverage{$id}{$pos} )
        {
          ...
          foreach  (sort {$a<=>$b} keys %backward) {
           ...
           my $cutseq=substr $refseq{$id}, $_, $tmplength;
           my $cutname=join '.', ($id, $_, $backward{$_}, $tmplength);
           print OUTG2 ">$cutname\n$cutseq\n";
       ...
       print OUTG1 "$id\tbased_on_coverage\tgene\t$_\t$backward{$_}\t.\t$strand\t0\tID=$tmpid;Length=$tmplength;SenseCoverge=$SenseCoverge;AntisenseCoverage=$AntisenseCoverage;CombineCoverage=$CombineCoverage;Direction=backward_strand;Antisense2Sense_ratio=$Antisense2Sense_ratio;gene_id=unknown$tmpid;locus_tag=unknown$tmpid\n";
          ...
          my $tmplength=$forward{$_}-$_+1;
           ...
           my $cutseq=substr $refseq{$id}, $_, $tmplength;
           my $cutname=join '.', ($id, $_, $forward{$_}, $tmplength);
           print OUTG2 ">$cutname\n$cutseq\n";
     ...
     print OUTG1 "$id\tbased_on_coverage\tgene\t$_\t$forward{$_}\t.\t$strand\t0\tID=$tmpid;Length=$tmplength;SenseCoverge=$SenseCoverge;AntisenseCoverage=$AntisenseCoverage;CombineCoverage=$CombineCoverage;Direction=forward_strand;Antisense2Sense_ratio=$Antisense2Sense_ratio;gene_id=unknown$tmpid;locus_tag=unknown$tmpid\n";
        ...
        foreach  (sort {$a<=>$b} keys %mixdir) {
           ...
           my $cutseq=substr $refseq{$id}, $_, $tmplength;
           my $cutname=join '.', ($id, $_, $mixdir{$_}, $tmplength);
       #    print OUTG2 ">$cutname\n$cutseq\n";
  ...
  close OUTG1;
  if (-e "$workdir/sum_direction_prokaryote_ref.gff")
   {
       foreach (@gfffile)
        {
          my $tmpfile=$_;  
         if($tmpfile) {  `cat $workdir/sum_direction_prokaryote_ref.gff >> $tmpfile`;}
        }   
   ...
   while (my $tmpdir = readdir(DIR))
   {
       ...
       next if ($tmpdir =~ /^\./);
       my $tmpgff="$workdir/differential_gene/prokaryote/$tmpdir/prokaryote.gff";
 
    if($tmpgff) { `cat $workdir/sum_direction_prokaryote_ref.gff >> $tmpgff`;}
...
open (OUTG1,">$workdir/newprokaryotegffmade.txt") or die " can not open description file $workdir/newprokaryotegffmade.txt$!";
print OUTG1 "done\n";
