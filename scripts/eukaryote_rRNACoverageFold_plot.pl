...
open ( LOG, ">>", $process_log_file) or die "failed: failed to write $process_log_file\n$!";
 ...
 my $genedection=join '.', ('direction_eukarya_ref', $sample,  'gff');
...
my $headfile="$workdir/eukarya.fa.fai"; # samtool head file
...
#`cp $workdir/eukarya.fa.fai $workdir/coverage.fa.fai`; 

if(&file_check("$workdir/eukarya.fa.fai")<0 ) {print LOG" failed$workdir/eukarya.fa.fai doesn't exist\n"; exit;}
if(&file_check("$workdir/$sample/mapping_results/forward.eukarya_ref.bam")<0 &&  &file_check("$workdir/$sample/mapping_results/backward.eukarya_ref.bam")<0 && &file_check("$workdir/$sample/mapping_results/forward.eukarya_ref.sam")>0 &&  &file_check("$workdir/$sample/mapping_results/backward.eukarya_ref.sam")>0)
 {

print LOG "
samtools view -bt $workdir/eukarya.fa.fai  $workdir/$sample/mapping_results/backward.eukarya_ref.sam  > $workdir/$sample/mapping_results/backward.eukarya_ref.bam\n
samtools view -bt $workdir/eukarya.fa.fai  $workdir/$sample/mapping_results/forward.eukarya_ref.sam  > $workdir/$sample/mapping_results/forward.eukarya_ref.bam\n
";

`samtools view -bt $workdir/eukarya.fa.fai  $workdir/$sample/mapping_results/backward.eukarya_ref.sam  > $workdir/$sample/mapping_results/backward.eukarya_ref.bam`;
`samtools view -bt $workdir/eukarya.fa.fai  $workdir/$sample/mapping_results/forward.eukarya_ref.sam  > $workdir/$sample/mapping_results/forward.eukarya_ref.bam`;
#unlink "$workdir/$sample/mapping_results/backward.eukarya_ref.sam";
#unlink "$workdir/$sample/mapping_results/forward.eukarya_ref.sam";
}
if(&file_check("$workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.bam")<0 &&  &file_check("$workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.bam")<0 && &file_check("$workdir/$sample/mapping_results/forward.eukarya_ref.bam")>0 &&  &file_check("$workdir/$sample/mapping_results/backward.eukarya_ref.bam")>0)  
  {
...
#unlink "$workdir/$sample/mapping_results/forward.eukarya_ref.bam";
        ...
        open GSZ, ">$workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.genomeSize" or die $!;
        open REF, "samtools view -H $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.bam |" or die $!;
        while(<REF>){
            if( /SN:(\S+)\tLN:(\d+)/ ){
                print GSZ "$1\t$2\n";
            }
        ...
        open GSZ, ">$workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.genomeSize" or die $!;
        open REF, "samtools view -H $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.bam |" or die $!;
        while(<REF>){
            if( /SN:(\S+)\tLN:(\d+)/ ){
                print GSZ "$1\t$2\n";
            }
...
genomeCoverageBed -split -bg -ibam $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.bam -g $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.genomeSize  > $workdir/$sample/mapping_results/forward.sorted.eukarya_ref.bedgraph\n
genomeCoverageBed -split -bg -ibam $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.bam -g $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.genomeSize  > $workdir/$sample/mapping_results/forward.sorted.eukarya_ref.bedgraph\n

";

`genomeCoverageBed -split -bg -ibam $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.bam -g $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.genomeSize  > $workdir/$sample/mapping_results/forward.sorted.eukarya_ref.bedgraph`;
#`wigToBigWig $workdir/$sample/mapping_results/forward.sorted.eukarya_ref.bedgraph $workdir/$sample/mapping_results/sortedpos.forward.eukarya_ref.genomeSize   $workdir/Jbrowse/BigWig/$sample.forward.sorted.eukarya_ref.bw`;

print LOG "
genomeCoverageBed -split -bg -ibam $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.bam -g $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.genomeSize  > $workdir/$sample/mapping_results/backward.sorted.eukarya_ref.bedgraph\n";
#wigToBigWig $workdir/$sample/mapping_results/backward.sorted.eukarya_ref.bedgraph $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.genomeSize  $workdir/Jbrowse/BigWig/$sample.backward.sorted.eukarya_ref.bw\n



`genomeCoverageBed -split -bg -ibam $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.bam -g $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.genomeSize  > $workdir/$sample/mapping_results/backward.sorted.eukarya_ref.bedgraph`;
#`wigToBigWig $workdir/$sample/mapping_results/backward.sorted.eukarya_ref.bedgraph $workdir/$sample/mapping_results/sortedpos.backward.eukarya_ref.genomeSize  $workdir/Jbrowse/BigWig/$sample.backward.sorted.eukarya_ref.bw`; 
if ($doeufinding ne 'yes') 
{
`pwd > $workdir/$sample/mapping_results/done.eukarya.txt`;
exit;
}
...

{
     ...
     #  my $tmpratio;
      ...
      $tmpname=~s/\|/-/g;
       ...
       my $forwardfile=join '.', ('forward_eukarya_ref', $sample, $tmpname, 'txt');
       my $backwardfile=join '.', ('backward_eukarya_ref', $sample, $tmpname, 'txt'); 
       my $ratiofile=join '.', ('ratio_eukarya_ref', $sample, $tmpname, 'txt');
       my $rscriptfile=join '.', ('Rscript_eukarya_ref', $sample, $tmpname, 'txt');
       my $plotfile=join '.', ('eukarya_ref_plot', $sample, $tmpname, 'jpg');
...

  {
    ...
    $deletion++;
        ...
        my $genedeletion=join '.', ('deletion_eukarya_ref', $sample, $tmpname, 'txt');
...
&coverageplot($forwardfile, $backwardfile, $ratiofile, $rscriptfile, $plotfile);
  ...
  ($window_size, $step_size)=(100,10);
   ...
   ($window_size, $step_size)=(1000,100);
            ...
            my $wdforwardfile=join '.', ('window_plot',$window_size,$step_size,'forward_eukarya_ref', $sample, $tmpname, 'txt');
       my $wdbackwardfile=join '.', ('window_plot',$window_size,$step_size,'backward_eukarya_ref', $sample, $tmpname, 'txt');
       my $wdratiofile=join '.', ('window_plot',$window_size,$step_size,'ratio_eukarya_ref', $sample, $tmpname, 'txt');
       my $wdrscriptfile=join '.', ('window_plot',$window_size,$step_size,'Rscript_eukarya_ref', $sample, $tmpname, 'txt');
       my $wdplotfile=join '.', ('window_plot',$window_size,$step_size,'eukarya_ref_plot', $sample, $tmpname, 'jpg');
