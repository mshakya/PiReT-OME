...
my ( $pairedReadsFile1, $indexFile, $pairedReadsFile2, $unpairedReadsFile,
    $workdir, $sampleDir, $mapDir, $ref );
my $Bowtie2Opts = "--fast";
...
&checkFiles( $indexFile, $pairedReadsFile1, $pairedReadsFile2,
    $unpairedReadsFile );
...
&runMapping( $indexFile, $pairedReadsFile1, $pairedReadsFile2,
    $unpairedReadsFile, $mapDir );
...
sub checkFiles {
    ...
    $indexFile_r .= ".1.ht2l";
    if ( -z $indexFile_r || !-e $indexFile_r ) {
        die "Index file $indexFile_r file is empty or does not exist";
    }

    if ( $#queryPairedFile1 != $#queryPairedFile2 ) {
        die "mate1 and mate2 have diffrent number of files ";
    }
    foreach my $queryPairedFile1 (@queryPairedFile1) {
        if ( defined $queryPairedFile1 ) {
            if ( -z $queryPairedFile1 || !-e $queryPairedFile1 ) {
                die "mate1 $queryPairedFile1 file is empty";
            }
        }
    }
    ...
    foreach my $queryPairedFile2 (@queryPairedFile2) {
        if ( defined $queryPairedFile2 ) {
            if ( -z $queryPairedFile2 || !-e $queryPairedFile2 ) {
                die "mate2 $queryPairedFile2 file is empty";
            }
        }
    }
    ...
    foreach my $queryUnpairedFile (@queryUnpairedFile) {
        if ( defined $queryUnpairedFile ) {
            if ( -z $queryUnpairedFile || !-e $queryUnpairedFile ) {
                die "$queryUnpairedFile file is empty";
            }
        }
    }
}
