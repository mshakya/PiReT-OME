    ...
    if ($queryPairedFile_r1 && $queryPairedFile_r2)
    {
         ...
         for ( my $i=0; $i <=  $#queryPairedFile1; $i++)
        {
         ...
         if (-e "$workdir/differential_gene/eukarya/splice_sites_gff.txt") 
           {
           $command = "hisat2 $Bowtie2Opts --known-splicesite-infile $workdir/differential_gene/eukarya/splice_sites_gff.txt   -p $numCPU -x $IndexFile -1 $queryPairedFile1 -2 $queryPairedFile2  2>$mappingLogFile| ";
           } else {
               $command = "hisat2 $Bowtie2Opts -p $numCPU -x $IndexFile -1 $queryPairedFile1 -2 $queryPairedFile2  2>$mappingLogFile| "; 
           }
            print " $command\n"; 
            open (my $fh, "$command") or die "$! hisat2 $Bowtie2Opts failed\n";
            while (<$fh>)
            {
                chomp;
                next if (/^\@/);
