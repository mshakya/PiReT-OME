...
$|=1;
$ENV{PATH} = "$Bin:$Bin/../:$Bin/hisat-0.1.5-beta/:$Bin/script/:$Bin/bin/:$ENV{PATH}:/cm/shared/apps/sge/2011.11p1/bin/linux-x64:$Bin/../../edge_ui/JBrowse/bin/";
my $main_pid = $$;
my $version="1.1";
...
open(STDERR, '>&', STDOUT) or die "failed: failed: Can't redirect stderr: $!";
open(STDERR, '>', $error_log_file) or die "failed: failed: Can't redirect stderr: $!";
...
unless (-d "$workdir/sum_gene_count/") {mkdir "$workdir/sum_gene_count/", 0777 or die "failed: failed: can not make dir  $workdir/sum_gene_count/ $!";}
unless (-d "$workdir/logdir/") {mkdir "$workdir/logdir/", 0777 or die "failed: failed: can not make dir  $workdir/dir/ $!";}
...
open (IN, "$descriptfile") or die "failed: failed: can not open description file $descriptfile $!";
 my $linenum=0;
 while (<IN>)
{
         chomp;
        my @line=split /\s+/, $_;
       unless  ($#line >=2) { &lprint ("check format of the description file $_ \n"); exit;}
        $linenum++;
      if ($linenum ==1)
   {
         for (my $i=0; $i<=$#line; $i++)
...
#
     ...
     for (my $ill=0; $ill<=$#line1; $ill++)
  {
   ...
   if(&file_check($tmpreads)<0 ) { die "failed: The reads file $_ doesn't exist or empty.\n";}
 ...
 push @{$pairs{$line[2]}}, $line[0];
   if ($line[2] =~ /\s+/) {
   &lprint ("failed: please check format of experimental descpition \n group name can not have space as : $line[2]\n");
   exit;
  }
...
if ( (($test_method eq 'both') || ($test_method eq 'Deseq') ) && $#{$pairs{$tmpgroup}}<2)
 {
   &lprint ("failed: testing method using Deseq2 but the $tmpgroup has only $pairs{$tmpgroup} which is less than 3. please not using Deseq method or increase the number of dunplcates for $tmpgroup\n");
 exit;
 }
}
...
unless (-d "$workdir/differential_gene/") {mkdir "$workdir/differential_gene/", 0777 or die "failed: failed: can not make dir  $workdir/differential_gene/ $!";}

unless (-d "$workdir/sum_gene_count/tmp_count/") {mkdir "$workdir/sum_gene_count/tmp_count/", 0777 or die "failed: failed: can not make dir  $workdir/sum_gene_count/tmp_count/ $!";}
unless (-d "$workdir/sum_gene_count/read_count/") {mkdir "$workdir/sum_gene_count/read_count/", 0777 or die "failed: failed: can not make dir  $workdir/sum_gene_count/read_count/ $!";}
...
if ($eukarya_fasta) 
 {
  if(&file_check($eukarya_fasta)<0  ) 
   {
     die "failed: failed: The eukarya_fast file $eukarya_fasta doesn't exist or empty.\n";
   } else {
   if (-e "$workdir/eukarya.fa") { `rm $workdir/eukarya.fa`;}
    ...
    if (&file_check("$workdir/eukarya.fa.fai")<0) {`samtools faidx $workdir/eukarya.fa`;}  
...
if ($prokaryote_fasta) 
 {
   ...
   { 
    die "failed: failed: The prokaryote_fast file $prokaryote_fasta doesn't exist or empty.\n";
   } else {
   if (-e "$workdir/prokaryote.fa") {`rm $workdir/prokaryote.fa`;}
    ...
    # check if the index file exists, do not run, if it already exists
    if (&file_check("$workdir/prokaryote.fa.fai")<0) {`samtools faidx $workdir/prokaryote.fa`;}  
...
foreach (keys %allcontigs) { if ($allcontigs{$_}>1) {&lprint ("contig name $_ is dubplicated, please make sure the name of contigs is unique in reference fasta files (prokaryout and eukarya references)\n"); exit;} }
...
if ($coverage_fasta)
 {
   ...
   { die  "failed: The coverage_fasta file $coverage_fasta doesn't exist or empty.\n";
   } else {
   ...
   `samtools faidx $pjcoverage`;
       my @contigs=&readfai("$workdir/prokaryote.fa.fai");
    foreach (@contigs) { unless ($allcontigs{$_}) {&lprint ("coverage contig $_ is not part of mapping index references, please make sure it is included in either prokaryout or eukarya reference\n"); exit;}}
   }
 } else {$pjcoverage='NA'}
...
{
      unless (-d "$workdir/sum_gene_count/tmp_count/eukarya") {mkdir "$workdir/sum_gene_count/tmp_count/eukarya", 0777 or die "failed: failed: can not make dir  $workdir/sum_gene_count/tmp_count/eukarya $!";}
     unless (-d "$workdir/sum_gene_count/read_count/eukarya") {mkdir "$workdir/sum_gene_count/read_count/eukarya", 0777 or die "failed: failed: can not make dir  $workdir/sum_gene_count/read_count/eukarya $!";}
     unless (-d "$workdir/differential_gene/eukarya") {mkdir "$workdir/differential_gene/eukarya", 0777 or die "failed: failed: can not make dir  $workdir/differential_gene/eukarya $!";}
 ...
 foreach (@tmpgff) { 
     ...
     { 
       next &lprint ("The gff_eukarya file $tmpgff doesn't exist or empty.\n");
     }
...
#        `$Bin/../../edge_ui/JBrowse/bin/flatfile-to-json.pl --gff $tmpgff --type gene  --tracklabel gene --out $workdir/Jbrowse`;
     ...
     unless (-d "$workdir/sum_gene_count/tmp_count/eukarya/$tmpeukarya[-1]") {mkdir "$workdir/sum_gene_count/tmp_count/eukarya/$tmpeukarya[-1]", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/tmp_count/eukarya/$tmpeukarya[-1] $!";}
     unless (-d "$workdir/sum_gene_count/read_count/eukarya/$tmpeukarya[-1]") {mkdir "$workdir/sum_gene_count/read_count/eukarya/$tmpeukarya[-1]", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/read_count/eukarya/$tmpeukarya[-1] $!";}
...
{
     unless (-d "$workdir/sum_gene_count/tmp_count/prokaryote") {mkdir "$workdir/sum_gene_count/tmp_count/prokaryote", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/tmp_count/prokaryote $!";}
     unless (-d "$workdir/sum_gene_count/read_count/prokaryote") {mkdir "$workdir/sum_gene_count/read_count/prokaryote", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/read_count/prokaryote $!";}
     unless (-d "$workdir/differential_gene/prokaryote") {mkdir "$workdir/differential_gene/prokaryote", 0777 or die "failed: can not make dir  $workdir/differential_gene/prokaryote $!";}
    ...
    my @tmpgff=split /,/, $gff_prokaryote; 
     ...
     $tmpprokaryote[-1] =~ s/gff//g;
       if(&file_check($tmpgff)<0 )      {
        &lprint ( "The gff_prokaryote file $tmpgff doesn't exist or empty.\n");
     }
...
#        `$Bin/../../edge_ui/JBrowse/bin/flatfile-to-json.pl --gff $tmpgff --type gene  --tracklabel gene --out $workdir/Jbrowse`; 
     ...
     unless (-d "$workdir/sum_gene_count/tmp_count/prokaryote/$tmpprokaryote[-1]") {mkdir "$workdir/sum_gene_count/tmp_count/prokaryote/$tmpprokaryote[-1]", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/tmp_count/prokaryote/$tmpprokaryote[-1] $!";}
     unless (-d "$workdir/sum_gene_count/read_count/prokaryote/$tmpprokaryote[-1]") {mkdir "$workdir/sum_gene_count/read_count/prokaryote/$tmpprokaryote[-1]", 0777 or die "failed: can not make dir  $workdir/sum_gene_count/read_count/prokaryote/$tmpprokaryote[-1] $!";}
     unless (-d "$workdir/differential_gene/prokaryote/$tmpprokaryote[-1]") {mkdir "$workdir/differential_gene/prokaryote/$tmpprokaryote[-1]", 0777 or die "failed: can not make dir  $workdir/differential_gene/prokaryote/$tmpprokaryote[-1] $!";}
     &lprint ("perl $Bin/parse_prokaryote_gfffile.pl $tmpgff  $workdir/differential_gene/prokaryote/$tmpprokaryote[-1]/ $workdir/prokaryote.fa.fai \n");
...
if ($pairfile)
 ...
 if(&file_check($pairfile)<0 ) 
   { die "failed: The given pair file $pairfile  doesn't exist or empty.\n";
   } else {
   my $linenum=0;
   open (IN, "$pairfile") or die "failed:  can not open  file $pairfile $!";
 while (<IN>)
{
            chomp;
        my @line=split /\t/, $_;
       unless  ($#line ==1) { &lprint (" failed: check format of the $pairfile file \n"); exit;}
        $linenum++;
        if ($linenum >1)
  {
     ...
     push @{$pair1{$line[0]}}, $line[1];
      } else { &lprint ("failed: $line[0] or $line[1] not defined in experimental descpition\n"); exit
      }
   } 
 }
}
...
&lprint ("perl $scriptDir/illumina_fastq_QC.pl  -min_L 60 -n 5 -q 15  -lc 0.7  -t $numCPU  -prefix $sample  -d  $workdir/$sample/trimming_results/ -p  $rawreads\n");
...
&lprint ("perl $scriptDir/rRNA_reads_mapping.pl -cpu $numCPU  -p1  $workdir/$sample/trimming_results/$sample.1.trimmed.fastq -p2 $workdir/$sample/trimming_results/$sample.2.trimmed.fastq -prefix $sample -index $indexref    -o $workdir \n");
...
&lprint ("perl $scriptDir/parse_BAMfile.pl  -bamfile $rawreads -sample $sample    -o $workdir\n");
...
my $sample=$_;
 if ($prokaryote_fasta) {
&lprint("perl $scriptDir/prokaryote_rRNACoverageFold_plot.pl  $sample   $workdir\n");
   ...
   while ($alldone)
  {
      ...
      # if ($eukarya_fasta && $prokaryote_fasta)
   {
     if (&file_check($tmpfile1)>0 && &file_check($tmpfile2)>0  ) {
        $alldone--;
       print LOG"done samples : $alldone\n";
      } else { print LOG "eukarya and  prokaryote rRNACoverageFold $sample not done\n";}
   } elsif  ( $prokaryote_fasta) {

        if (&file_check($tmpfile2)>0  ) {
        $alldone--;
       print LOG"done samples : $alldone\n";
      } else { print LOG "prokaryote rRNACoverageFold $sample not done\n";}
...
&lprint ("perl $Bin/prokaryote_find_small_rna.pl $workdir $prokaryote_fasta  $gff_prokaryote $allsample\n");
...
foreach (sort keys %description ) {
my $sample=$_;
open (READIN, ">$workdir/sum_gene_count/read_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt") or die "failed: $! $workdir/sum_gene_count/read_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt\n"; 
     open (IN, "$workdir/sum_gene_count/tmp_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt") or die "failed:  can not open description file $workdir/sum_gene_count/tmp_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt $!";
        while (<IN>)
          {
                chomp;
                my $line=$_;
...
unless (-d "$Deseqdir/Deseq/") {mkdir "$Deseqdir/Deseq/", 0777 or die "failed: can not make dir  $workdir/differential_gene/$kingdom/$strain/Deseq/ $!";}
unless (-d "$Deseqdir/EdgeR/") {mkdir "$Deseqdir/EdgeR/", 0777 or die "failed: can not make dir  $workdir/differential_gene/$kingdom/$strain//EdgeR/ $!";}
unless (-d "$Deseqdir/figures/") {mkdir "$Deseqdir/figures/", 0777 or die "failed: can not make dir  $workdir/differential_gene/$kingdom/$strain//figures/ $!";}
unless (-d "$Deseqdir/significant_gene/") {mkdir "$Deseqdir/significant_gene/", 0777 or die "failed: can not make dir  $workdir/differential_gene/$kingdom/$strain//significant_gene/ $!";}



my %tablecounts;
open (DESC, ">$Deseqdir/reads.table.txt") or die "failed: $! $Deseqdir/reads.table.txt\n";
print DESC "Sample";
my %reads_sample;
my %reads_desc;
...
foreach (sort keys %description ) {
my $sample=$_;
      open (IN, "$workdir/sum_gene_count/read_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt") or die "failed:  can not open description file $workdir/sum_gene_count/read_count/$kingdom/$strain/$sample.$kingdom.name.htseq.locus_tag.txt $!";
        while (<IN>)
          {
                chomp;
                my $line=$_;
...
foreach (sort keys %reads_sample) 
 ...
 print DESC "\n";
if ($#goodsample<1) {&lprint ("no good sample for $kingdom, $strain\n"); next;}
$allgoodsample++;
...
open (DESC, ">$Deseqdir/readcounts.expriment.txt") or die "failed: $! $Deseqdir/readcounts.expriment.txt\n";
...
open (DESC, ">$Deseqdir/Deseq_EdgeRpairs.txt") or die "failed: $! $Deseqdir/Deseq_EdgeRpairs.txt\n";
...
if ($kingdom eq 'prokaryote') 
 {
 &lprint ("perl $Bin/Differential_stats_prokaryote.pl $workdir $Deseqdir $descriptfile $workdir/differential_gene/$kingdom/$strain/prokaryote.NonrRNA.genedesc.txt  $p_cutoff @goodsample\n");
...
if ($kingdom eq 'eukarya')
 {
&lprint ("perl $Bin/Differential_stats_eukarya.pl $workdir $Deseqdir $descriptfile $workdir/differential_gene/$kingdom/$strain/eukarya.genedesc.txt  $p_cutoff @goodsample\n");
...
if ($allgoodsample>0)
{
    ...
    &lprint ("  Done Differential Gene Analysis \n");
     ...
     &lprint ("  Differential Gene Analysis failed because of no good samples \n");
     &printRunTime($time3);
    &lprint ("  failed to do Differential Gene Analysis failed because of no good samples \n");
}
...
open (CURRENTLOGFILE,  "> $workdir/process_current.log") or die "   $workdir/process_current.log $!";
