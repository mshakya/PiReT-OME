...
while (<$fh>) {
    ...
    if ( $_ =~ /\tgene\t/ || $_ =~ /\texon\t/ ) {
        ...
        foreach ( sort keys %seqhash ) {
            ...
            my $tmp2 = $tmpln[0];
            $tmp2 =~ s/\|//g;
            if ( $tmp1 =~ /$tmp2/ || $tmp2 =~ /$tmp1/ ) { $tmpln[0] = $_; }
        }
        ...
        for ( my $i = 0; $i <= $#tmpname; $i++ ) {
            ...
            unless ( $tmpname[$i] =~ /=/ ) {
                print
                    "please check gff file $gfffile format at line $linecount \n $tmpln[-1]\n";
        ...
        unless ($gene_id) {
            if   ($GeneID) { $gene_id = $GeneID; }
            else           { $gene_id = $tmpname; }
        ...
        $tmpname =~ s/"//g;
        $tmpname =~ s/gene\=//g;
        if ($description) { $genedesc{description}{$tmpname} = $description; }
        else              { $genedesc{description}{$tmpname} = 'unknown'; }
...
open( DESC, ">$workdir/eukarya.genedesc.txt" )
    or die "$! $workdir/eukarya.genedesc.txt\n";
foreach ( sort { ( $genedesc{gene_id}{$a} cmp $genedesc{gene_id}{$b} ) }
    keys %{ $genedesc{gene_id} } )
{
