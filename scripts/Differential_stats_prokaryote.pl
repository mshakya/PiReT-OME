...
close IN;
my %groupNUM;
foreach (@allsample) { my $tmpgroup=$_;  $groupNUM{$replicates{$tmpgroup}}++;}
        ...
        while (<$fh>)
            {
               ...
               my $tmpline=$_;
               my @tmpline=split /\t/, $_;
                my @tmpln=split /___/, $tmpline[2];
                my @tmpln1=split /---/, $tmpln[-1];
                $geneln{$tmpline[0]}=$tmpln1[1]-$tmpln1[0]+1;
                $genedes{$tmpline[0]}=$tmpline[1];
            }
       close $fh;


open (PATHIN, "$Bin/kegg_locus_orgnism.txt") or die "$! $Bin/kegg_locus_orgnism.txt  \n";
        ...
        while (<COUNTIN>)
         {
               ...
               else
            {
                ...
                $readcounts_gene{$genename{$linecount}}{$samplename[$i-1]}=$tmpline[$i];
                 $totalcounts_sample{$samplename[$i-1]}+=$tmpline[$i];
                }
            }
          }
       close COUNTIN;
...
#         print "$workdir/$sample/mapping_results/$sample.stats.text\n";
        ...
        if (-e "$workdir/$sample/trimming_results/$sample.stats.txt") {
        ...
        close COUNTIN;
         ...
         $total_reads{$sample}=$trim_reads{$sample};
        }
   ...
   {
    unless ($geneln{$genename{$i}}) {next; print LOGFILE "$genename{$i}\n";}
   print RPKMOUT "$genename{$i}";
   for (my $k=0; $k<=$#samplename; $k++)  
      {
        my $counts=0;
      #  print "$samplename[$k]\t$readcounts_gene{$genename{$i}}{$samplename[$k]}\t";
      #  print "$totalcounts_sample{$samplename[$k]}\n"; 
     ...
     # print "$genename{$i}\t$samplename[$k]\t$readcounts_gene{$genename{$i}}{$samplename[$k]}\t$counts\n";
  ...
  my $existdeseq=0;
   ...
   {
        if ($edgfile =~ m/--sig\.txt$/) 
       {
        my $tmpfile=$edgfile;
        $tmpfile=~s/--sig\.txt/\.txt/;
     ...
     #read over Deseq
      ...
      open ( COUNTIN, "$diffgendir/Deseq/$desfile ") or die "$! $diffgendir/Deseq/$desfile  \n";
            while (<COUNTIN>)
         {
               ...
               #splice (@tmpline, 3,1);
               foreach (@tmpline) {$_=~s/\"//g;}
              # print "0: $#tmpline: @tmpline\n";
               if ($linecountsig==1) 
               {
                 splice (@tmpline, 3,1);
                 # print "1: $#tmpline: @tmpline\n";
                for (my $i=0; $i<=$#tmpline; $i++)
                {
   ...
   close RPKMOUT;
     $heatmapfig=join ".", ($heatmapfile, 'pdf');
      ...
      #find Intercestion 
       ...
       my $sumfile=join "_", ('Sum_allgenes','EdgeR', $desfile, '.xls');
...
unless (-d "$diffgendir/significant_gene/pathway/$pathwayfile/") {mkdir "$diffgendir/significant_gene/pathway/$pathwayfile/", 0777 or die "$diffgendir/significant_gene/pathway/$pathwayfile $!";}
   ...
   # chdir "$diffgendir";
   # print LOGFILE"cd to $diffgendir\n";
    print LOGFILE"Rscript $Bin/scatt_plot.R  $RPKMscatfile $diffgendir\n";
    `/usr/bin/Rscript $Bin/scatt_plot.R   $RPKMscatfile $diffgendir`;
 ...
 unless ($geneln{$genename} ) {next;}
  ...
  print SUMOUT "::$genename\t$genedes{$genename}";
   ...
   close RPKMOUT;
    $heatmapfig=join ".", ($heatmapfile, 'pdf');
    print LOGFILE "$Bin/heatmap_distinctZ_noClust_zeroRowAllow.py --in $diffgendir/significant_gene/$heatmapfile -s log --out $diffgendir/figures/$heatmapfig\n";
   ` $Bin/heatmap_distinctZ_noClust_zeroRowAllow.py --in $diffgendir/significant_gene/$heatmapfile -s log --out $diffgendir/figures/$heatmapfig`;
    ...
    print LOGFILE "cd to $diffgendir/significant_gene/pathway/$pathwayfile/\n";
    print LOGFILE "/usr/bin/Rscript $Bin/pathview.R  \n";
 
    chdir "$diffgendir/significant_gene/pathway/$pathwayfile/";
 `/usr/bin/Rscript $Bin/pathview.R  `;
 #`perl $Bin/json.pl`;

    } else { #end if $workdir/differential_gene/Deseq/$desfile
       print LOGFILE"$diffgendir/Deseq/$desfile does not exist\n";
   }
...
print LOGFILE"cd to $diffgendir/significant_gene/pathway/all_comparison\n";
 ...
 if ($existdeseq <1 && $existedger>=1)
 {
   ...
   #read over EdgeR
     my $linecountsig=0;
      ...
      my $sumfile=join "_", ('Sum_allgenes', $edgfile, '.xls');
     open ( SUMOUT, ">$diffgendir/$sumfile") or die "$! $diffgendir/$sumfile  \n";
...
unless (-d "$diffgendir/significant_gene/pathway/$pathwayfile/") {mkdir "$diffgendir/significant_gene/pathway/$pathwayfile/", 0777 or die "$diffgendir/significant_gene/pathway/$pathwayfile $!";}
 open ( PATHOUT, ">$diffgendir/significant_gene/pathway/$pathwayfile/pathway.stats.txt") or die "$! $diffgendir/significant_gene/pathway/$pathwayfile/pathway.stats.txt  \n";
   ...
   foreach (@{$pathway{$genename}} )
  {
   ...
   chdir "$diffgendir/significant_gene/pathway/$pathwayfile/";
   print LOGFILE "/usr/bin/Rscript --vanilla --slave --silent  < $Bin/pathview.R  2>/dev/null\n";
 `/usr/bin/Rscript  $Bin/pathview.R  `;
# `perl $Bin/json.pl`;
 ...
 if ($existdeseq >=1 && $existedger < 1)
 {
   ...
   #read over Deseq
     my $linecountsig=0;
      ...
      my $sumfile=join "_", ('Sum_allgenes', $desfile, '.xls');
     open ( SUMOUT, ">$diffgendir/$sumfile") or die "$! $diffgendir/$sumfile  \n";
...
close PATHOUT;
print "/usr/bin/Rscript  $Bin/pathview.R  ";
`/usr/bin/Rscript  $Bin/pathview.R  `;
#`perl $Bin/json.pl`;
