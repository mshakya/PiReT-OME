...
map_data <- read.delim(file = "pathway.rpkm.txt", stringsAsFactors = FALSE,  header=TRUE)
...
limitscale=log10(max(unique_row_set))
...
#pv.out<-pathview(new.signature = FALSE, out.suffix = outsuff,limit= list(gene = limitscale, cpd = limitscale), bins = list(gene = 40, cpd = 40), trans.fun= list(gene =log10, cpd=log10), gene.data=de_matrix[,1:sample_size], gene.idtype="kegg", pathway.id=pathwayid, species=speciesid, kegg.native = T, same.layer=T)
pv.out<-pathview(new.signature = FALSE, out.suffix = outsuff, both.dirs = list(gene = F, cpd = F), limit= list(gene = limitscale, cpd = limitscale), bins = list(gene = 40, cpd = 40), trans.fun= list(gene =log10, cpd=log10), gene.data=de_matrix[,1:sample_size], gene.idtype="kegg", pathway.id=pathwayid, species=speciesid, kegg.native = T, same.layer=T, mid=list(gene = "green", cpd = "green"),)
  }
}
