...
#significant
#if (nrow(filename_sig_deseq)==0) next
#filename_sig_deseq <-  subset(filename_deseq,filename_deseq$pvalue<pcutoff)
#write.table(filename_sig_deseq,sep = "\t",  file=fileout_sig_Deseq)
...
filename_EdgeR<-topTags( filename_EdgeR, n = nrow( filename_EdgeR$table ) , sort.by = "p.value" )$table
