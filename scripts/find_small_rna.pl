...
{
my $sample=$_;
unless (  (-e "$workdir/$sample/mapping_results/forward.sorted.coverage.bedgraph") && (-e "$workdir/$sample/mapping_results/backward.sorted.coverage.bedgraph") ) {next;}
my $forwardpileupfile="$workdir/$sample/mapping_results/forward.sorted.coverage.bedgraph";#=$ARGV[1]; # samtools pileup snp and indel call output 

my $backwardpileupfile="$workdir/$sample/mapping_results/backward.sorted.coverage.bedgraph"; #=$ARGV[2];
...
open (OUTG1,">$workdir/sum_direction_coverage.gff") or die " can not open description file $workdir/sum_direction_coverage.gff $!";
foreach  (sort keys %forwardcoverage)
{
...

  {
             ...
             if ( $forwardcoverage{$id}{$pos}>$mincov || $backwardcoverage{$id}{$pos}>$mincov )
    {
        ...
        foreach  (sort {$a<=>$b} keys %mixdir) {
           ...
           print OUTG1 "$id\tbased_on_coverage\tgene\t$_\t$mixfdir{$_}\t.\t$strand\t0\tID=$tmpid;Length=$tmplength;ForwardCoverge=$ForwardCoverage;BackwardCoverage=$BackCoverage;CombineCoverage=$CombineCoverage;Direction=undef;gene_id=unknown$tmpid;locus_tag=unknown$tmpid\n";
         print OUTG1 "$id\tbased_on_coverage\texpressed_intergenic_region\t$_\t$mixfdir{$_}\t.\t$strand\t0\tID=$tmpid;Length=$tmplength;ForwardCoverge=$ForwardCoverage;BackwardCoverage=$BackCoverage;CombineCoverage=$CombineCoverage;Direction=undef;gene_id=unknown$tmpid;locus_tag=unknown$tmpid\n";
             }
         }
    ...
    #     print OUTG1 "$id\tbased_on_coverage\texpressed_RNA_region\t$_\t$mixbdir{$_}\t.\t$strand\t0\tID=$tmpid;Length=$tmplength;SenseCoverge=$SenseCoverage;AntisenseCoverage=$AntisenseCoverage;CombineCoverage=$CombineCoverage;Direction=back_strand;Antisense2Sense_ratio=$Antisense2Sense_ratio\n";
     #  }
